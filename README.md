...
По умолчанию первый успешный проход создаёт baseline без алертов. `notify_existing_on_first_run = true` отправит все найденные объявления после очистки базы. Далее бот отправляет новые объявления и изменения цены. До 10 фотографий группируются в один альбом; телефон приводится к формату `+374...`. Авторизация и подготовленные cookies не нужны.
